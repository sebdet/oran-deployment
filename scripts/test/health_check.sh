...
checkStatus "curl -skw %{http_code} http://$a1_sim_OSC_url/" "OK200" "SIM1"

# check SIM2 status
echo "check SIM2 status:"
checkStatus "curl -skw %{http_code} http://$a1_sim_STD_url/" "OK200" "SIM2"

# check SIM3 status
echo "check SIM3 status:"
checkStatus "curl -skw %{http_code} http://$a1_sim_STD_v2_url/" "OK200" "SIM3"

# check PMS status
echo "check PMS status:"
checkStatus "curl -skw %{http_code} http://$policy_agent_url/status" "hunky dory200" "PMS"

# check ECS status
echo "check ECS status:"
checkStatus "curl -skw %{http_code} http://$enrichment_service_url/status" '{"status":"hunky dory","no_of_producers":0,"no_of_types":0,"no_of_jobs":0}200' "ECS"
