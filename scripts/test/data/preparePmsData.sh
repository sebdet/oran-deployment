...
# Run command:
# ./preparePmsData.sh [a1-sim-OSC host] [a1-sim-STD-2 host] [http/https]

policy_agent_port=30094
a1_sim_OSC_port=${1:-localhost}
a1_sim_STD_port=${2:-localhost}
httpx=${3:-"http"}
SHELL_FOLDER=$(cd "$(dirname "$0")";pwd)

echo "using policy_agent port: "$policy_agent_port
echo "using a1-sim-OSC host: "$a1_sim_OSC_port
echo "using a1-sim-STD host: "$a1_sim_STD_port
...
echo "ric1 version:"
curlString="curl -skw %{http_code} $httpx://$a1_sim_OSC_port:8085/counter/interface"
res=$($curlString)
echo "$res"
expect="OSC_2.1.0200"
checkRes
...
echo "ric2 version:"
curlString="curl -skw %{http_code} $httpx://$a1_sim_STD_port:8085/counter/interface"
res=$($curlString)
echo "$res"
expect="STD_2.0.0200"
checkRes
...
curlString="curl -X PUT -skw %{http_code} $httpx://$a1_sim_OSC_port:8085/policytype?id=1 -H Content-Type:application/json --data-binary @${SHELL_FOLDER}/testdata/OSC/policy_type.json"
res=$($curlString)
echo "$res"
...
curlString="curl -skw %{http_code} $httpx://$a1_sim_STD_port:8085/policytype?id=2 -X PUT -H Accept:application/json -H Content-Type:application/json -H X-Requested-With:XMLHttpRequest --data-binary @${SHELL_FOLDER}/testdata/v2/policy_type.json"
res=$($curlString)
echo "$res"
...
curlString="curl -skw %{http_code} $httpx://$a1_sim_OSC_port:8085/counter/num_instances"
res=$($curlString)
echo "$res"
expect="1200"
checkRes
...
curlString="curl -skw %{http_code} $httpx://$a1_sim_STD_port:8085/counter/num_instances"
res=$($curlString)
echo "$res"
expect="1200"
checkRes
